...
[t_new z]=ode15s(@pathway_model_many_antibody_immune_res_new,[0 500],x0,[],para,para_new);




for i = 1:100
data_new_no_self(i,:) = interp1(t_new,z(:,i),(0:1:500));
end

data_k_on_no_self = zeros(10, 501);
...
for i = 1:100
    ...
    % 将 data_new 的当前列加到对应的 data_k_on 行
    data_k_on_no_self(row_index, :) = data_k_on_no_self(row_index, :) + data_new_no_self(i, :);
end


data_k_off_no_self = zeros(10, 501);
...
for i = 1:100
    ...
    % 将 data_new 的当前列加到对应的 data_k_off 行
    data_k_off_no_self(row_index, :) = data_k_off_no_self(row_index, :) + data_new_no_self(i, :);
end

data_kd_no_self = zeros(19, 501);
...
for i = 1:100
    ...
    % 将 data_new 的当前列加到对应的 data_kd 行
    for j = 1:501
        data_kd_no_self(row_index, j) = data_kd_no_self(row_index, j) + data_new_no_self(i, j);
    end
end
...
% 创建视频写入对象
writerObj = VideoWriter('extended_video_4.avi');
open(writerObj);
